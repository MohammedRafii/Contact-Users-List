  ...
  const fetchAllContacts = async () => {
    ...
    try {
      ...
      if (response.data.user) {
        ...
        setTotalContacts(response.data.user.length)
        setLoading(false)
      } else {
        return null
      }
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center w-full bg-gradient-to-r from-sky-500 to-indigo-500">
      ...
      grid-cols-[repeat(auto-fit,minmax(300px,1fr))] gap-6 mx-auto">
        ...
        )) : <div className="text-white/80 text-2xl">There is no data</div>)}
      </main>
      <div className="mx-auto flex items-center py-2 text-2xl">
        <div className="flex items-center mx-auto">
          ...
          <div className="flex mx-auto m-4  text-xl">
            {[...Array(Math.ceil(contacts?.length / 7))].map((_, i) => {
          ...
          <span onClick={() => selectPageHandler(page + 1)} className={`${page < Math.ceil(contacts.length / 7) ? "" : "hidden"} cursor-pointer ml-2 font-medium hover:underline active:underline`}>Next</span>
        </div>
      </div>
      <Outlet />
    </div >
